...
class ModelTrainer:
    ...
    def _calculate_regime_metrics(self, X: pd.DataFrame, y: pd.Series, 
                                 meta_features: pd.DataFrame, oof_df: pd.DataFrame) -> Dict:
        """Calculate performance by market regime"""
        try:
            # Define market regimes based on volatility and returns
            if 'market_volatility' in meta_features.columns:
                vol_col = 'market_volatility'
            elif 'market_sma_10' in meta_features.columns:
                vol_col = 'market_sma_10'  # Proxy for volatility
            else:
                # Create simple volatility proxy
                returns_col = y.pct_change().abs().rolling(20).mean()
                vol_col = 'vol_proxy'
                meta_features[vol_col] = returns_col
            
            # Define regimes
            regimes = pd.cut(
                meta_features[vol_col].fillna(meta_features[vol_col].median()),
                bins=3,
            ...
            for regime in ['Low_Vol', 'Medium_Vol', 'High_Vol']:
                ...
                regime_pred = oof_df[regime_mask].mean(axis=1) if not oof_df[regime_mask].empty else pd.Series()
                
                if len(regime_pred) > 0:
                    regime_ic = spearmanr(regime_y, regime_pred)[0] if len(regime_y) > 0 else 0
                    regime_mse = mean_squared_error(regime_y, regime_pred)
                    
                    regime_metrics[regime] = {
                        'ic': float(regime_ic) if not np.isnan(regime_ic) else 0.0,
                        'mse': float(regime_mse),
                        'samples': int(regime_mask.sum()),
                        'period_pct': float(regime_mask.sum() / len(regimes) * 100)
                    }
            
            logger.info(f"📊 Regime analysis: {len(regime_metrics)} regimes identified")
    ...
    def _calculate_turnover_metrics(self, oof_df: pd.DataFrame, meta_features: pd.DataFrame) -> Dict:
        """Calculate portfolio turnover and capacity metrics"""
        try:
            if oof_df.empty:
                return {}
            
            # Calculate signal changes (proxy for turnover)
            ensemble_signal = oof_df.mean(axis=1)
            
            # Convert to positions (long/short/neutral)
            positions = pd.Series(0, index=ensemble_signal.index)
            positions[ensemble_signal > 0.52] = 1  # Long
            positions[ensemble_signal < 0.48] = -1  # Short
            
            # Calculate turnover (position changes)
            position_changes = positions.diff().abs()
            daily_turnover = position_changes.mean()
            
            # Estimate capacity based on turnover
            # Lower turnover = higher capacity
            estimated_capacity_mm = max(1, 100 / (daily_turnover * 252)) if daily_turnover > 0 else 1000
            
            # Calculate holding periods
            position_groups = (positions != positions.shift()).cumsum()
            holding_periods = positions.groupby(position_groups).count()
            avg_holding_period = holding_periods.mean()
            
            turnover_metrics = {
                'daily_turnover': float(daily_turnover),
                'annual_turnover': float(daily_turnover * 252),
                'estimated_capacity_mm': float(min(estimated_capacity_mm, 1000)),  # Cap at $1B
                'avg_holding_period_days': float(avg_holding_period),
                'total_position_changes': int(position_changes.sum()),
                'long_signals': int((positions == 1).sum()),
                'short_signals': int((positions == -1).sum()),
                'neutral_periods': int((positions == 0).sum())
            }
            
            logger.info(f"📈 Turnover analysis: {daily_turnover:.3f} daily, {estimated_capacity_mm:.0f}M capacity")
