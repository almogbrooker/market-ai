...
def pick_latest_institutional_model(root_path="institutional_test_results"):
    """Pick the latest institutional testing model"""
    root = Path(root_path)
    if not root.exists():
        raise FileNotFoundError(f"No institutional results found at {root_path}")
    
    # Look for latest date directory
    date_dirs = []
    for d in root.iterdir():
        if d.is_dir() and re.match(r'202\d{3}', d.name):
            date_dirs.append(d)
    
    if not date_dirs:
        raise FileNotFoundError("No dated result directories found")
    
    latest_date_dir = sorted(date_dirs)[-1]
    
    # Find best model configuration within the date directory
    model_dirs = []
    for d in latest_date_dir.iterdir():
        if d.is_dir() and (d / "model.pt").exists():
            # Prefer all_features or balanced configurations
            score = 0
            if "all_features" in d.name:
                score += 10
            elif "balanced" in d.name:
                score += 8
            elif "technical" in d.name:
                score += 6
            elif "sentiment" in d.name:
                score += 4
            elif "fundamental" in d.name:
                score += 2
            
            model_dirs.append((score, d))
    
    if not model_dirs:
        raise FileNotFoundError(f"No model directories found in {latest_date_dir}")
    
    return sorted(model_dirs, reverse=True)[0][1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--models_root", default="institutional_test_results", 
